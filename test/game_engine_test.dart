...
GameState buildState({
  ...
  List<ActionCard> hand(PlayerId owner) => [
        ...
        ActionCard(id: '${owner.name}_prot', type: ActionType.protect, owner: owner),
      ];
  return GameState(
    humans: humans,
...
HumanCard human(int pos, HumanType type, int points,
        {CardState state = CardState.alive, bool protected = false}) =>
    HumanCard(
        id: 'h$pos',
        position: pos,
        type: type,
...
void main() {
  ...
  group('保カード', () {
    ...
    test('保護中に死を受けると保護だけ壊れ、生死は不変', () {
      var s = buildState(humans: [
        human(0, HumanType.good, 3, protected: true),
        ...filler(),
      ]);
      s = engine.applyAction(s, act(s, ActionType.death, 0));
      final h = s.humanAt(0);
  ...
  group('ゲーム終了', () {
    test('両者の手札が尽きたら finished', () {
      var s = buildState(humans: filler()..insert(0, human(0, HumanType.good, 3)));
